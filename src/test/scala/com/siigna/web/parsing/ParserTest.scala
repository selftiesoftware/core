...
class ParserTest extends FlatSpec with Matchers {
  ...
  it should "parse a line" in {
    ...
    Parser.parse(stream) should equal (Right(SeqExpr(Seq(RefExpr("line", Seq(ConstantExpr(0), ConstantExpr(0), ConstantExpr(10), ConstantExpr(10)))))))
  }
  ...
  it should "parse text" in {
    ...
    Parser.parse(stream) should equal (Right(SeqExpr(Seq(RefExpr("text", Seq(ConstantExpr(0), ConstantExpr(0), ConstantExpr(10), ConstantExpr("Hello world")))))))
  }

  it should "parse circles" in {
    val stream = Lexer.lex("circle 40 60 20")
    Parser.parse(stream) should equal (Right(SeqExpr(Seq(RefExpr("circle", Seq(ConstantExpr(40), ConstantExpr(60), ConstantExpr(20)))))))
  }

  it should "parse arcs" in {
    val stream = Lexer.lex("arc 100 60 20 2 4")
    Parser.parse(stream) should equal (Right(SeqExpr(Seq(RefExpr("arcs", Seq(ConstantExpr(100), ConstantExpr(60), ConstantExpr(20), ConstantExpr(2), ConstantExpr(4)))))))
  }
